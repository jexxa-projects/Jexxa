...
| Driving Adapter
| A driving adapter belongs to the infrastructure and receives a command from a specific client such as a UI, or a console and forwards it to a specific port. Within Jexxa a driving adapter is able to automatically expose the methods from all kind of ports by using a convention. In cases where this is not possible, Jexxa offers a so called __Inbound Port Adapter__.
...
| An inbound port adapter belongs to the infrastructure and maps incoming requests to a dedicated port. This is typically required if a mapping between different styles such as a mapping between resources and methods as for RESTfulHTTP.
