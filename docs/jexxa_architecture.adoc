...
and rebuild the application. Unfortunately, you hide the flow of control which makes it harder for beginners to understand an application which is based on a ports and adapters architecture. This is especially
This might be obvious to incoming synchronous calls (RMI), but can be hard to see for incoming asynchronous messaging.

IMPORTANT: Jexxa uses constructor injection together with explicit binding for the driving adapter to make the flow of control visible. The main method represents the unique starting point of the flow of control.
