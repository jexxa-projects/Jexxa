...
As described in Section <<Motivation>>, the problem scope is typically not on the technical side. The big challenge is to understand the business domain and what our customers or users want to achieve which is called `value`. In contrast to technology companies, companies from production, financial or insurance sector do not create `value` by using different technology stacks without changing their business model and thus their business applications. At most, they can reduce their costs by using different technology stacks or technology provider.

This actually leads to a rather obvious conclusion about team structures, which in my experience is often ignored:

* Your senior developers should focus on the application core and support your customers to map a specific domain to a suitable structure for the application core.
* The intermediate developers then support implementing the application core.
* Since your senior and intermediate developers focus on the application core, your junior developers have to focus on the technology stack. Therefore, your framework should be as simple as possible and not an `egg-laying wool-milk sow`.

An additional advantage is that the training time of new developers is drastically reduced until they can be productive.
The training period for a new business domain typically takes 1-2 years, whereas the training period for a simple framework is a couple of weeks.
Therefore, they can support their colleagues very quickly by implementing the infrastructure part.

IMPORTANT: Much more important is that junior developers get the time to learn the craftsmanship of software development, methods of software
architecture and the domain language step by step from the intermediate and senior developers.

Unfortunately, I have often experienced in the past that companies expect a graduate or junior developer to already know the craftsmanship of software engineering and
methods of software architecture so that they only need to learn a business domain.

Jexxa was developed to ideally support the described team structure. The rest of this section described the resulting design consequences and
why conventions or configurations are used.
...
Most today's frameworks bind technology stacks automatically to your application core. If at all, you have to add a new dependency and rebuild the application. Unfortunately, you hide the flow of control which makes it harder for beginners to understand an application which is based on a ports and
...
=== Strategy pattern for Driven Adapter

Driven adapters belong to the infrastructure and are application specific because the implement outbound ports. Thus, their implementation should be as simple as possible so that they can be implemented by junior developers.

IMPORTANT: Jexxa provides `driven adapter strategies` to implement driven adapters so that their implementation is just a simple facade, which maps between the API of outbound ports to corresponding API of the strategy.

This seems to be so obvious so we directly agree. Therefore, I would like to explain the most important advantages of using the strategy pattern especially for the training of new developers. As an example I will use the implementation of a repository in the sense of DDD, which manages so called aggregates:
...
As soon as your junior developers feel that they are not challenged with implementing driven adapters as described above, give them one of these points to study.

=== Super simple API

The implementation of In order to enforce dependency inversion, all driven adapter must implement an interface which resides inside the application core.

using strategy patterns.

In case the technology stacks of your application becomes legacy, a developer has to analyze which technology stacks are used by the application. Since
he has only to switch

This leads to three major decisions:

* A super simple API to include arbitrary technology stacks without updating the core library



* The possibility to bind driving technology stacks on a per object level. This supports following cases:
** You should migrate a legacy system but during ths phase your new developed application must communicate with the legacy system using some real old communication mechanism. In this case you should only expose the realy required objects and not all. Otherwise, there is a fair chance that the developers maintaining the legacy system will use the new cool features from the new application and you will end in a neverending migration process.
** You can evaluate new technology stacks, or if the application itself is legacy you can add some required external binding but only to objects theirs functionality is realy required.


Due to these reasons, Jexxa requires an explicit binding for driving adapters in all cases.

To enable this functionality as well for driven adapter which are only interfaces and must be implemented, Jexxa uses strategy pattern, so that implementing a driven adapter means
* querying a strategy for a specific object
* mapping methods from application by forwarding it to interface of the startegy similiar to a facade.
