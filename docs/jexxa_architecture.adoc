...
* Your senior developers should focus on the application core and support your customers to map a specific domain to a suitable structure for the
application core.
* The intermediate developers then support implementing the application core.
* Since your senior and intermediate developers focus on the application core, your junior developers have to focus on the technology stack. Therefore,
your framework should be as simple as possible and not an `egg-laying wool-milk sow`.

IMPORTANT: Using a simple framework ensures that junior developers get the time to learn the craftsmanship of software development, methods of software
architecture and the domain language step by step from the intermediate and senior developers.

Unfortunately, I have often experienced in the past that companies expect a graduate or junior developer to already know the craftsmanship of software
engineering and methods of software architecture so that they only need to learn a business domain.

Jexxa was developed to ideally support the above team structure. Even though if you have complex application scenarios that requires a sophisticated
framework, you should use some lightwight frameworks to support the integration of new developers. The rest of this section described the resulting
design consequences and why conventions or configurations are used.
