...
[JexxaMain] -left-> [Dependency Injection]
...
Available strategies in Jexxa are based on the standard javax interfaces (e.g. JMS or JDBC) and can be configured via `Properties` and allows you to adjusted them to your development process. For example, you can easily switch your database technology between in memory, JDBC, or key value without changing your driven adapters.
...
a| JexxaMain
a| This component includes the `JexxaMain` which is used in the main-method of your application.
