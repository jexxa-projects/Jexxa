...
a| Jexxa-Core
a| Provides the core functionality of Jexxa as described in <<Jexxa Features>>. In case you write a new business application,
you will need this package. It also provides driving and driven adapter based on java standard APIs such as:

* Driving-Adapter: `JMXAdapter`, `JMSAdapter`
* Driven-Adapter Strategies: `JDBCKeyValueRepository`, `JMSSender`

a| Jexxa-Web
a| Provides driving adapters to access your application via HTTP. The used web-server is jetty.
