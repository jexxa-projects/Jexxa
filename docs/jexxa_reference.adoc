...
a|`destination`
a| Name of the jms topic or queue.

a|`selector`
a| Defines a message selector to filter messages. Default value is `""`.

a|`messagingType`
a| Must be either `TOPIC` or `QUEUE` depending on the used messaging type.
